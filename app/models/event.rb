class Event
  ...
  has_mongoid_attached_file :logo, :styles => { :small => "50x50>", :medium => "75x75!", :large => "200x200>", :extra_large => "450x190!" }
  ...
  def as_json(options = {})
    options = {:only => [:_id, :name, :featured], :methods => [:upcoming_event, :past?]} 
    super
  end
